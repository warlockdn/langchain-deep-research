...
async def get_tokens(config: RunnableConfig):
    ...
    namespace = _get_token_namespace(config)
    if not namespace:
        return None

    # Retrieve stored tokens
    tokens = await store.aget((namespace, "tokens"), "data")
    if not tokens:
        return None
    ...
    if current_time > expiration_time:
        ...
        await store.adelete((namespace, "tokens"), "data")
        return None
...
async def set_tokens(config: RunnableConfig, tokens: dict[str, Any]):
    ...
    # Store the tokens
    await store.aput((namespace, "tokens"), "data", tokens)


def _get_token_namespace(config: RunnableConfig) -> str | None:
    """Resolve the namespace used for persisted MCP tokens."""
    metadata = config.get("metadata", {})
    configurable = config.get("configurable", {})

    return metadata.get("owner") or configurable.get("thread_id")
